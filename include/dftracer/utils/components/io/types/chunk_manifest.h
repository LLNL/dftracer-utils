#ifndef DFTRACER_UTILS_COMPONENTS_IO_TYPES_CHUNK_MANIFEST_H
#define DFTRACER_UTILS_COMPONENTS_IO_TYPES_CHUNK_MANIFEST_H

#include <dftracer/utils/components/io/types/chunk_spec.h>
...
namespace std {
template <>
struct hash<dftracer::utils::components::io::ChunkManifest> {
    std::size_t operator()(const dftracer::utils::components::io::ChunkManifest&
                               manifest) const noexcept {
        std::size_t h1 = std::hash<double>{}(manifest.total_size_mb);
        std::size_t h2 = 0;
        for (const auto& spec : manifest.specs) {
            h2 ^=
                std::hash<dftracer::utils::components::io::ChunkSpec>{}(spec) +
...
#endif  // DFTRACER_UTILS_COMPONENTS_IO_TYPES_CHUNK_MANIFEST_H
