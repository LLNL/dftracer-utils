...
static int test_bytes_stream_byte_range(void) {
    ...
    // Create stream
    dft_reader_stream_t stream = dft_reader_stream(
        reader, DFT_STREAM_TYPE_BYTES, DFT_RANGE_TYPE_BYTES, 0, 100);
...
static int test_bytes_stream_line_range(void) {
    ...
    dft_reader_stream_t stream = dft_reader_stream(
        reader, DFT_STREAM_TYPE_BYTES, DFT_RANGE_TYPE_LINES, 1, 10);
    CHECK_NOT_NULL(stream, "stream");
...
static int test_line_bytes_stream(void) {
    ...
    dft_reader_stream_t stream = dft_reader_stream(
        reader, DFT_STREAM_TYPE_LINE_BYTES, DFT_RANGE_TYPE_LINES, 1, 10);
    CHECK_NOT_NULL(stream, "stream");
...
static int test_multi_lines_bytes_stream(void) {
    ...
    dft_reader_stream_t stream = dft_reader_stream(
        reader, DFT_STREAM_TYPE_MULTI_LINES_BYTES, DFT_RANGE_TYPE_LINES, 1, 20);
    CHECK_NOT_NULL(stream, "stream");
...
static int test_line_stream(void) {
    ...
    // Create LINE stream
    dft_reader_stream_t stream = dft_reader_stream(reader, DFT_STREAM_TYPE_LINE,
                                                   DFT_RANGE_TYPE_LINES, 6, 15);
    CHECK_NOT_NULL(stream, "stream");
...
static int test_multi_lines_stream(void) {
    ...
    dft_reader_stream_t stream = dft_reader_stream(
        reader, DFT_STREAM_TYPE_MULTI_LINES, DFT_RANGE_TYPE_LINES, 10, 30);
    CHECK_NOT_NULL(stream, "stream");
...
static int test_stream_recreation(void) {
    ...
    size_t bytes1;
    {
        dft_reader_stream_t stream1 = dft_reader_stream(
            reader, DFT_STREAM_TYPE_BYTES, DFT_RANGE_TYPE_BYTES, 0, 100);
    ...
    {
        dft_reader_stream_t stream2 = dft_reader_stream(
            reader, DFT_STREAM_TYPE_BYTES, DFT_RANGE_TYPE_BYTES, 0, 100);
...
static int test_edge_cases(void) {
    ...
    dft_reader_stream_t stream1 = dft_reader_stream(
        reader, DFT_STREAM_TYPE_BYTES, DFT_RANGE_TYPE_BYTES, 100, 100);
    CHECK_NOT_NULL(stream1, "stream1");

    char buffer[128];
    size_t bytes = dft_reader_stream_read(stream1, buffer, sizeof(buffer));
    CHECK(bytes == 0, "empty range should read 0 bytes");
    CHECK(dft_reader_stream_done(stream1), "empty range should be finished");
    dft_reader_stream_destroy(stream1);
    ...
    while (!dft_reader_stream_done(stream2)) {
        size_t b =
            dft_reader_stream_read(stream2, tiny_buffer, sizeof(tiny_buffer));
        if (b == 0) break;
        total += b;
    }
    ...
    size_t first_bytes;
    {
        dft_reader_stream_t stream3 = dft_reader_stream(
            reader, DFT_STREAM_TYPE_BYTES, DFT_RANGE_TYPE_BYTES, 0, 50);
